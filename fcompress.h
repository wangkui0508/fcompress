 ...
 *
 * 默认路径: 量化在 f16 域完成 (FSUB.8H/FMUL.8H/FCVTNU.8H), 比 f32 路径快约 1.9x,
 * q 与参考实现最多差 1 个 LSB.
 * 以下两种情况会自动回退 f32 量化 (只有 q 变精确, min/step 不受影响):
 *   a) 某列 step 小到 1/step 会溢出 f16 (range < ~5.1e-3);
 *   b) 某列跨度 mx-min 超过 f16 最大值 (range > ~65504), 否则 v-min 在 f16 域
 *      溢出成 inf, 该列顶部整段被饱和到 q=255.
 * 回退是块级的: 32 列里只要有 1 列命中, 整块都走慢路径 (编码约 1.9x 慢).
